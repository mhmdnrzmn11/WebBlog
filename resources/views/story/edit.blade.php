...
<div class="container">
    <div class="row">
        <div class="col-md-12">                            
            ...
            <hr class="mt-3">   
            <form action="{{ route('stories.update', $post->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col col-lg-6">
                        <div class="form-group my-3">
                            ...
                            <input type="text" name="title" id="title" class="w-100 form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" value="{{$post->title}}" required>
                            @if ($errors->has('title'))
                    ...
                    <div class="col col-lg-6">
                        <div class="form-group my-3">
                            ...
                            <select name="category" id="category" class="w-100 form-control{{ $errors->has('category') ? ' is-invalid' : '' }}" readonly>
                                <option value="{{$post->category->id}}" selected>{{$post->category->nama}}</option>
                            </select>
                    ...
                    <div class="col col-lg-12">                                
                        <div class="form-group mt-4">
                            <textarea id="text-content"  name="content" class="w-100 form-control{{$errors->has('content') ? ' is-invalid' : ''}}" id="content" rows="3" value="{{old('content')}}">{{$post->content}}</textarea>
